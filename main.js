...
\\end{figure}`;
      ...
      tex = tex.replace(reMdImgInTex, (_full, relPath) => {
        ...
        const abs = import_path2.default.join(vaultPath, cleanRel);
      ...
      for (const { abs, rel } of toCopy) {
        ...
        if (await import_fs_extra2.default.pathExists(abs)) {
          ...
          if (fallback && await import_fs_extra2.default.pathExists(fallback)) {
            console.log("[md2overleaf] image found via vault scan:", rel, fallback);
            await import_fs_extra2.default.copy(fallback, dest);
          } else {
            console.warn(
  ...
  async resolveMissingImage(rel, vaultPath) {
    try {
      const normalized = rel.replace(/\\/g, "/");
      const candidateName = import_path2.default.basename(normalized);
      if (!candidateName)
        return null;
      ...
      return import_path2.default.join(vaultPath, match.path);
    } catch (error) {
